...
**183. zdbview — first terminal UI browser for rkyv archives: typed key/value decode for registered archive types, structural fallback for the rest, in one binary that also does full SQLite CRUD** — `med`
rkyv archives are **not self-describing** — the format stores layout and relative pointer offsets, no field names and no type tags — so there has never been a `sqlitebrowser`-equivalent for them: with no schema in the file, a generic reader has nothing to bind columns to, and inspecting a cache shard means writing a throwaway Rust program that links the producer's own types. zdbview closes that for the archives this stack actually produces. It carries **faithful copies of the producer's archive types**, detects each by a magic word, validates with `rkyv::check_archived_root` — so a version/feature drift fails validation instead of silently decoding garbage — and renders real `(key, value)` records with per-entry scalar fields and a hex pane over the value blob; two zshrs cache formats are registered today (`ZRSC` script cache, `ZRAL` autoload cache). Anything unrecognized degrades to a structural view rather than an error: printable-string runs with byte offsets, plus an `xxd`-style hex/ascii dump. The same binary opens SQLite databases with **full generic CRUD** — browse tables with row counts, paginated rows, in-place cell edit, insert-with-defaults, rowid delete, arbitrary SQL — and picks the backend by **header magic, not filename** (`SQLite format 3\0` is authoritative; a `.db` whose bytes aren't a SQLite header is opened as an archive), with extension used only for files too short to carry a header. No-arg launch is an MRU picker off `$XDG_CACHE_HOME/zdbview/recent`. *Basis:* `zdbview/src/formats.rs` (`SCRIPT_MAGIC`/`AUTOLOAD_MAGIC`, the copied `ScriptShard`/`AutoloadShard` types pinned to rkyv 0.7 + `archive_le` + `size_32`, `try_decode` → `check_archived_root`, `Decoded`/`KvRecord`), `src/app.rs:121` (decode wired into the TUI; records + fields + hex panes), `src/rkyv_inspect.rs` (the `strings`/`hex_row` fallback), `src/store.rs:14-36` (`SQLITE_MAGIC`, magic-authoritative `detect`), `src/sqlite.rs` (rowid-addressed CRUD), `src/mru.rs`; ratatui 0.30 + rusqlite 0.40 (bundled). **Test-verified:** `cargo test` → 11 passing — `formats::tests::script_shard_roundtrips_through_try_decode` serializes a real shard and asserts the decoded format/records/header, `garbage_is_not_recognized` guards the fallback, `tests/backend.rs` covers `sqlite_full_crud_roundtrip`, `rkyv_structural_strings_and_hex`, both detection paths, and MRU dedup/order. *Caveat:* a database TUI is a known category (`sqlitebrowser`, `litecli`, `harlequin`) and the SQLite half claims nothing new; the candidate-first is the **rkyv half** — GitHub repo search (`rkyv viewer|inspector|tui|dump|browser`), `gh search code` (`ratatui rkyv`), a 60-result crates.io sweep, and three web searches surfaced only the rkyv library and its docs, no archive viewer. "None found," not proven; private and non-`rkyv`-named tools are outside what those indexes cover. Scope is honest: typed decode covers **two registered formats**, not arbitrary archives, and the rkyv side is **read-only** — CRUD over an archive needs a supplied schema descriptor and is deferred. Companion to the zshrs rkyv cache layer, whose shards it exists to inspect. MIT.
