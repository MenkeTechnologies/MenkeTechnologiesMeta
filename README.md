...
[![Submodules](https://img.shields.io/badge/submodules-79-blue.svg)](#0x01-submodule-map)
...
### `[META REPO // 79 SUBMODULES // ONE COMMAND, EVERY MENKETECHNOLOGIES PROJECT]`
...
The `--recurse-submodules` flag fetches all 79 submodules in parallel during the initial clone.
...
All 79 submodules sit flat at the repository root. URLs are HTTPS for fresh-host portability (no SSH key needed for `clone --recurse`).
...
| [`stryke-search`](https://github.com/MenkeTechnologies/stryke-search) | Elasticsearch / OpenSearch client — index admin, document CRUD, bulk, query DSL builders, scroll, aliases, cluster health. One client for both engines; pure-Rust `ureq`/rustls transport, no JVM. &middot; <sub>[docs](https://menketechnologies.github.io/stryke-search/) · [report](https://menketechnologies.github.io/stryke-search/report.html)</sub> |
