...
| [`vimlrs`](https://github.com/MenkeTechnologies/vimlrs) | VimL (Vimscript) in Rust — the first compiled **standalone** VimL interpreter, run outside Vim. A faithful port of Neovim's C eval engine (lex/parse → AST → `fusevm` bytecode → Cranelift JIT), the same VM behind `strykelang` / `zshrs` / `awkrs`. Standalone binary, with a DAP debugger, LSP, and rkyv bytecode cache planned. **Early** — Vim-licensed (a faithful port inherits the Vim license). |
