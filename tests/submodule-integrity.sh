#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 107-submodule org-wide
...
if [[ $n -ne 107 ]]; then
    echo "FAIL  expected 107 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 107 submodule entries"
fi
