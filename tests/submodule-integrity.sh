#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 97-submodule org-wide
...
if [[ $n -ne 97 ]]; then
    echo "FAIL  expected 97 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 97 submodule entries"
fi
