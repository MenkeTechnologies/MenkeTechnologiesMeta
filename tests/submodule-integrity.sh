#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 132-submodule org-wide
...
if [[ $n -lt 135 ]]; then
    echo "FAIL  expected >135 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has >135 submodule entries"
fi
