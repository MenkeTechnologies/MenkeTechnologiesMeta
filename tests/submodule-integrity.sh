#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 73-submodule org-wide
...
if [[ $n -ne 73 ]]; then
    echo "FAIL  expected 73 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 73 submodule entries"
fi
