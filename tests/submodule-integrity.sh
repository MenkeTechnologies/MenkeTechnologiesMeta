#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 125-submodule org-wide
...
if [[ $n -ne 125 ]]; then
    echo "FAIL  expected 125 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 125 submodule entries"
fi
