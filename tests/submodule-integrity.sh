#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 91-submodule org-wide
...
if [[ $n -ne 91 ]]; then
    echo "FAIL  expected 91 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 91 submodule entries"
fi
