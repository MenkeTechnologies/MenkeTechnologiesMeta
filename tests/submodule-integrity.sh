#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 99-submodule org-wide
...
if [[ $n -ne 99 ]]; then
    echo "FAIL  expected 99 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 99 submodule entries"
fi
