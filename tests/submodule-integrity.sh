#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 79-submodule org-wide
...
if [[ $n -ne 79 ]]; then
    echo "FAIL  expected 79 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 79 submodule entries"
fi
