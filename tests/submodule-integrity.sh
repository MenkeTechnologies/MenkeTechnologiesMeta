#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 130-submodule org-wide
...
if [[ $n -ne 130 ]]; then
    echo "FAIL  expected 130 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 130 submodule entries"
fi
