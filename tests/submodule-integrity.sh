#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 68-submodule org-wide
...
if [[ $n -ne 68 ]]; then
    echo "FAIL  expected 68 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 68 submodule entries"
fi
