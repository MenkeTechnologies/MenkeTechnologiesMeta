#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 90-submodule org-wide
...
if [[ $n -ne 90 ]]; then
    echo "FAIL  expected 90 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 90 submodule entries"
fi
