#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 70-submodule org-wide
...
if [[ $n -ne 70 ]]; then
    echo "FAIL  expected 70 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 70 submodule entries"
fi
