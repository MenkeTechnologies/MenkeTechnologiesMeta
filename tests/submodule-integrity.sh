#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 126-submodule org-wide
...
if [[ $n -ne 126 ]]; then
    echo "FAIL  expected 126 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 126 submodule entries"
fi
