#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 113-submodule org-wide
...
if [[ $n -ne 113 ]]; then
    echo "FAIL  expected 113 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 113 submodule entries"
fi
