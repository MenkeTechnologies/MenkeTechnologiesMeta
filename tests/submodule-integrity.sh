#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 65-submodule org-wide
...
if [[ $n -ne 65 ]]; then
    echo "FAIL  expected 65 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 65 submodule entries"
fi
