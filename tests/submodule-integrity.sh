#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 131-submodule org-wide
...
if [[ $n -ne 131 ]]; then
    echo "FAIL  expected 131 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 131 submodule entries"
fi
