#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 100-submodule org-wide
...
if [[ $n -ne 100 ]]; then
    echo "FAIL  expected 100 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 100 submodule entries"
fi
