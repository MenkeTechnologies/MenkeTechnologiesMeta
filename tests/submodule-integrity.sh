#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 87-submodule org-wide
...
if [[ $n -ne 87 ]]; then
    echo "FAIL  expected 87 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 87 submodule entries"
fi
