#!/usr/bin/env bash
# Pins structural invariants on .gitmodules so the 88-submodule org-wide
...
if [[ $n -ne 88 ]]; then
    echo "FAIL  expected 88 submodule entries, got $n"
    ok=0
else
    echo "PASS  .gitmodules has 88 submodule entries"
fi
